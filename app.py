...
def fetch_coupang_orders(access_key: str, secret_key: str, vendor_id: str):
    ...
    # ① 서명용 쿼리 스트링 생성 (URL과 완전히 동일해야 함)
    query_string = _make_query_string(params)

    # ② Authorization 헤더 생성
    auth, _ = _generate_auth_header(method, path, query_string, access_key, secret_key)
    ...
    # ③ 동일한 query_string으로 요청 (requests가 재인코딩하지 않도록 URL에 직접 삽입)
        ...
        if st.button("🔄 쿠팡 API 실시간 동기화", use_container_width=True, type="primary"):
            ...
            if not ak or not sk or not vid:
                ...
                with st.spinner("쿠팡 API 연결 중…"):
                    try:
                        orders = fetch_coupang_orders(ak, sk, vid)
